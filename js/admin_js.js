  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (status == true) {
        ...
        Message.forEach(function (user_name, i) {
          html += `<option value="${user_name["appointment_id"]},${user_name["patient_id"]}">${user_name["patient_name"]} - ${user_name["patient_tell"]}</option>`;
        });
...
$(document).on("change", "#examCheckAll", function () {
  if ($(this).is(":checked")) {
    ...
    if (checkNoCheckBox <= 0) {
      ...
      $(".allExamTest :checkbox").prop("checked", true);
      $("#regExam").html(
        '<i class="fa fa-send"></i> Register Lab Selected All'
      );
    }
  } else {
    $("#showHideExamBtn").hide();
    $(".allExamTest :checkbox").prop("checked", false);
    $("#regExam").html('<i class="fa fa-send"></i> Register Lab Selected All');
  }
});
...
$(document).on("change", "#allowedLab", function () {
  ...
  if (checkAllEmpCheck <= 0) {
    ...
    $("#showHideExamBtn").show();
    $("#examCheckAll").prop("checked", false);
    $("#regExam").html('<i class="fa fa-send"></i> Register Lab Selected');
  }
});
var app_Number = [];
var app_pateintID = [];
$("#registerLabExam").on("submit", function (e) {
  e.preventDefault();
  var app_data = $("#cmbPatientNameAppointment").val();
  var b1 = app_data.split(",");
  app_Number.push(b1[0]);
  app_pateintID.push(b1[1]);
  var selectedExam = "";
  $("input[name='allowedLab']:checked").each(function () {
    selectedExam += this.value + "###";
  });

  var embToBeCharged = selectedExam.slice(0, -3);
  var post = {
    action: "labExamTest_Appointmentor",
    embToBeCharged: embToBeCharged,
    salChargeMonth: "hjh",
  };
  alert(app_Number);
