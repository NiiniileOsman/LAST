...
function fetchExamTest($conn)
{
	...
	?>
			...
			<div class="tile-body">
				<div class="container-fluid">
					...
					<div class="row mb-2">
						...
						<div class="col-md-1 text-right" style="padding: 7px 8px 5px 2px; cursor: pointer;" id="check_Select">
							<h6 class="text-info">
								<input type="checkbox" class="form-control" style="height: 17px; width: 17px; display: inline;cursor: pointer;" name="examCheckAll" id="examCheckAll">
								Select All
							</h6>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
							<table class="table table-striped allExamTest">
								<?php
								$getMain = mysqli_query($conn, "SELECT DISTINCT Ex_Cat_ID,Ex_Cat_Name FROM exam_catagory GROUP by Ex_Cat_Name ORDER BY Ex_Cat_Name ");
								if ($getMain) {
									$main_catagory = mysqli_num_rows($getMain);
									if ($main_catagory > 0) {
										while ($catagory_rs = mysqli_fetch_array($getMain)) {
								?>
											<tr>
												<td style="padding: 15px 0px 10px 25px;" class="#">

													<h5><i class="<?php echo $catagory_rs[2]; ?>"></i><b><?php echo $catagory_rs[1]; ?></b></h5>
												</td>
											</tr>
											<tr>
												<td>
													<div class="row">
														<?php
														$getSubMain = mysqli_query($conn, "SELECT DISTINCT  Ex_Cat_ID  , Ex_Cat_Description FROM exam_catagory  WHERE Ex_Cat_Name='" . $catagory_rs[1] . "'");

														if ($getSubMain) {
															$submenu_num = mysqli_num_rows($getSubMain);
															if ($submenu_num > 0) {
																while ($lab_rs = mysqli_fetch_array($getSubMain)) {
														?>

																	<div class="col-md-4" style="padding: 10px 10px 10px 50px;">

																		<h6 class="text-info">
																			<input type="checkbox" class="form-control" style="height: 17px; width: 17px; display: inline;cursor: pointer;" name="allowedLab" id="allowedLab" value="<?php echo $lab_rs[0]; ?>"  data-id="<?php echo $lab_rs[0]; ?>" data-src="<?php echo $currentAppointment; ?>">
																			<i class="<?php echo $lab_rs[2]; ?>"></i> &nbsp;&nbsp;<b><?php echo $lab_rs[1]; ?></b>
																		</h6>
								...
								<?php
										...
										echo "<h5>Sorry, no Patient Appointment found </h5>";
									}
								} else {
									echo mysqli_error($conn);
								}
			...
			<div class="tile-footer" style="text-align: right;" id="showHideExamBtn">
				<button type="Submit" class="btn btn-primary" name="regExam" id="regExam"><i class="fa fa-fw fa-lg fa-save"></i>Register Exam</button>&nbsp;&nbsp;&nbsp;
				<button type="reset" class="btn btn-danger" data-dismiss="modal" onclick="location.reload()"><i class="fa fa-fw fa-lg fa-times"></i>Close</button>
			</div>
